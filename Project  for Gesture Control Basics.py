...
while True:
    ...
    lower_skin = np.array([20, 255, 255], dtype=np.uint)
    upper_skin = np.array([20, 255, 255], dtype=np.uint)
    ...
    # Find contours (hand shape) in masked image
    contours, _ =
...
if contours:
    max_contour =
max(contours,
key=cv2.contourArea)  # Get largest contour
if cv2.contourArea(max_contour) >
500:
